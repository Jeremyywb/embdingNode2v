...
# def trs(fn):
#     f = open(fn)
#     fms0Line = f.readlines()
#     fms0Line = [inm.split(" ")[0:2] for inm in fms0Line] 
#     f.close()
#     fnew = open(fn.split(".")[0] + "_noweit_" + ".txt" ,'w')
#     for l in fms0Line:
#         fnew.write(" ".join(l) + "\n")
#     fnew.close()

deepwalk --format edgelist  --input /usr/local/glsample/device1_edglist_noweit_.txt \
--max-memory-data-size 0 --number-walks 80 --representation-size 36 --walk-length 40 --window-size 10 \
--workers 1 --output /usr/local/glsample/device1_edglist_noweit.embeddings